$(function () {
    ...
    $.ajaxSetup({
  		headers : {    	
    		'X-Shopify-Storefront-Access-Token' : '14836b732a987b90f24738b3bf6b5015',
	...
	// GraphQL response conversion

	function convertGraphQLtoProducts(data) {
		var conv_products = [];
		var fetched_products = data.shop.products.edges;
		for(var i = 0; i < fetched_products.length; i++) {
			var product = fetched_products[i].node;
			var p = {};
			p.id = product.id;
			p.name = product.title;
			p.description = product.description;
			p.price = product.variants.edges[0].node.price;
			p.image = {};
			p.image.small = product.images.edges[0].node.src;
			p.image.large = product.images.edges[0].node.src;
			p.specs = {};
			p.specs.manufacturer = product.vendor;
      		p.specs.storage = 16;
      		p.specs.os = "iOS";
      		p.specs.camera = 8;
			conv_products.push(p);
		}
		return conv_products;
	}
